...
class DelegateFragment : Fragment() {
        ...
        RxPermissions(this)
            ...
            .subscribe {
                if (it) {
                    ...
                    resultSubject.onNext(PlateRecognitionResult.Error("没有权限"))
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CODE) {
            ...
            if (plate == null) {
                resultSubject.onNext(PlateRecognitionResult.Error("识别取消"))
                resultSubject.onComplete()
            } else {
                ...
                resultSubject.onNext(
                    PlateRecognitionResult.Success(
                        plate,
                        bitmapToBase64(plateBitmap)
                    )
                )
                resultSubject.onComplete()
